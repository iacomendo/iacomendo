...
> **Nota de layout:** o `.env` e o `.env.example` ficam na **raiz do
> repositório** (são compartilhados com os outros projetos). Rode os comandos
> abaixo a partir desta pasta (`relatorios-automaticos/`); os `.py` deste
> projeto leem as variáveis do ambiente carregado.

**Dev local:**
```bash
cp ../.env.example ../.env      # preencha com os valores reais (fica na raiz)
...
Nunca são versionados (ver `.gitignore` na raiz): `token.json`,
`credentials.json`, `clientes.db`, CSVs, `.env`.
